...
- `guided_maze`: A maze with red and green boxes to help guide the robot to
  the goal.
- `maze`: A maze with walls.
- `maze_gems`: A maze with walls and gems.
- `race_track`: An arena with a race track mimicry.
- `race_track_traffic_signs`: An arena with with a race track mimicry. It also
  has traffic signs.
...
- `width`: The width of the maze, Must be an odd number. Default is `11`.
- `length`: The length of the maze, Must be an odd number. Default is `11`.
...
The starting positions for the race track world can be found in the `config`
directory.
